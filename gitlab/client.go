// Package gitlab provides convenience wrappers around the go-gitlab package.
package gitlab
...
func NewClient(hostname string, user string, pass string) (*Client, error) {
	...
	// }
	client := gitlab.NewClient(nil, strings.TrimSpace(pass))
	// If we're using gitlab.com then we don't need to do any additional configuration
	...
	if hostname != "gitlab.com" {
		...
		if err != nil {
			return nil, errors.Wrapf(err, "Invalid gitlabb hostname trying to parse %s", baseURL)
		}
		err2 := client.SetBaseURL(baseURL)
		if err != nil {
			return nil, errors.Wrapf(err2, "Error setting Gitlab base URL to %s", baseURL)
		}
	}
...
func (c *Client) GetModifiedFiles(repo models.Repo, pull models.PullRequest) ([]string, error) {
	...
	for {
		...
		////// UPTO HERE, changing over to go-gitlab library. ////////////
		pageFiles, resp, err := c.client.MergeRequests.ListFiles(c.ctx, repo.Owner, repo.Name, pull.Num, &opts)
		if err != nil {
			return files, err
		}
